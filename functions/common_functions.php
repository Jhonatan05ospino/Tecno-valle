...
function filterCategoryProduct()
{
    ...
    if (isset($_GET['category'])) {
        ...
        if ($num_of_rows == 0) {
            echo "
                <h2 class='text-center'>Sin stock para esta categoría</h2>
                ";
        }
        while ($row = mysqli_fetch_assoc($select_product_result)) {
            $product_id = $row['product_id'];
...
function filterBrandProduct()
{
    ...
    if (isset($_GET['brand'])) {
        ...
        if ($num_of_rows == 0) {
            echo "
                <h2 class='text-center'>Sin stock para esta marca</h2>
                ";
        }
        while ($row = mysqli_fetch_assoc($select_product_result)) {
            $product_id = $row['product_id'];
...
function search_product()
{
    ...
    if (isset($_GET['search_data_btn'])) {
        ...
        if ($num_of_rows == 0) {
            echo "
                <h2 class='text-center'>No hay resultados que coincidan. ¡No se encontró ningún producto!</h2>
                ";
        }
                            ...
                            <div class='delivery d-flex flex-column my-4 gap-3'>
                                ...
                                <div class='d-flex gap-2 align-items-center'>
                                    ...
                                    <div class='d-flex flex-column gap-2'>
                                        ...
                                        <span>Detalles. Devoluciones gratuitas en 30 días. </span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
function cart($num_of_items = 1)
{
    if (isset($_GET['add_to_cart'])) {
        ...
        if ($num_of_rows > 0) {
            echo "<script>alert('Este artículo ya está presente en el carrito');</script>";
            ...
            echo "<script>alert('Este  Producto ha sido agregado a tu carrito');</script>";
            echo "<script>window.open('products.php','_self');</script>";
        }
    }
}
            ...
            if (!isset($_GET['my_orders'])) {
                if (!isset($_GET['delete_account'])) {
                    ...
                    if($row_orders_count > 0){
                        echo "<h3 class='text-center mb-3'>You have <span class='text-2'>$row_orders_count</span> Orden Pendiente</h3>
