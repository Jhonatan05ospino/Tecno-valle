...
if (isset($_POST['insert_categ_title'])) {
    ...
    if ($numOfResults > 0) {
        echo "<script>alert('La categoría ya está en la base de datos');</script>";
    } else {

        $insert_query = "INSERT INTO `categories` (category_title) VALUES ('$category_title')";
        $insert_result = mysqli_query($con, $insert_query);
        if ($insert_result){
            echo "<script>alert('La categoría ha sido insertada con Exito');</script>";
        }
    }
}
?>
...
<form action="" method="POST" class="mb-2">
    ...
    <div class="input-group w-10 mb-3">
        <input type="submit" class="btn btn-primary" name="insert_categ_title" value="Insertar Categorias" aria-label="Username" aria-describedby="basic-addon1">
    </div>
</form>